...
export class RandomRectTexture extends RectTexture {
    ...
    addPoint(brush: SVGBrush, x: number, y: number): void {
        for (let i = 0; i < RandomRectTexture.BRUSH_OBJECT_NUMBER; i++) {
            const randX = this.randomDeviation(brush.lineWidth * RandomRectTexture.BRUSH_RECT_FACTOR);
            const randY = this.randomDeviation(brush.lineWidth * RandomRectTexture.BRUSH_RECT_FACTOR);
